...
const currentPrimaryId = computed(
    () => props.primarySelectedId || props.selectedIds.at(-1) || null,
);
const selectedBounds = computed(() =>
    getSelectionBounds(
        orderedWidgets.value.filter((item) => selectedIdSet.value.has(item.id)),
    ),
);
const scale = computed(() =>
    scaleMode.value === "fit" ? fitScale.value : manualScale.value,
);
const scaleLabel = computed(() => `${Math.round(scale.value * 100)}%`);
const isFitMode = computed(() => scaleMode.value === "fit");
const scaledWidth = computed(
    () => props.project.meta.screenWidth * scale.value,
);
const scaledHeight = computed(
    () => props.project.meta.screenHeight * scale.value,
);
const showRulers = computed(
    () =>
        !props.previewMode &&
        !props.runtimeMode &&
        props.project.meta.showRulers !== false,
);
const showGuides = computed(
    () =>
        !props.previewMode &&
        !props.runtimeMode &&
        props.project.meta.showGuides !== false,
);
const guideColor = computed(
    () => props.project.meta.guideColor || DEFAULT_GUIDE_COLOR,
);
...
const hasGuides = computed(
    () => verticalGuides.value.length > 0 || horizontalGuides.value.length > 0,
);
    ...
    const top =
        bounds.y > 42
            ...
            : Math.min(
                  ...
                  props.project.meta.screenHeight - 34,
              );
...
const verticalGuideItems = computed(() => {
    ...
    const hiddenIndex =
        guideDragState.value?.orientation === "vertical"
            ? guideDragState.value.index
            : null;
...
const horizontalGuideItems = computed(() => {
    ...
    const hiddenIndex =
        guideDragState.value?.orientation === "horizontal"
            ? guideDragState.value.index
            : null;
...
const horizontalRulerTicks = computed(() =>
    buildRulerTicks(props.project.meta.screenWidth),
);
const verticalRulerTicks = computed(() =>
    buildRulerTicks(props.project.meta.screenHeight),
);
...
function updateScale() {
    ...
    const horizontal =
        (bounds.width - reservedWidth) / props.project.meta.screenWidth;
    const vertical =
        (bounds.height - reservedHeight) / props.project.meta.screenHeight;
...
function buildMeasurementLines(bounds) {
    const lines = [];
    const preferredTop =
        bounds.y > 24 ? bounds.y - 14 : bounds.y + bounds.h + 14;
    const preferredLeft =
        bounds.x > 24 ? bounds.x - 14 : bounds.x + bounds.w + 14;
...
function ensureGuideCollections() {
    if (
        ...
        typeof props.project.meta.guides !== "object"
    ) {
        props.project.meta.guides = {
            vertical: [],
            horizontal: [],
...
function emitSelection(ids, primaryId = null) {
    ...
    const validIds = Array.from(new Set(expandedIds)).filter((id) =>
        findWidget(id),
    );
...
function handleWidgetSelect(payload) {
    const selection = resolveSelectionForInteraction(
        payload.widgetId,
        payload.event,
    );
...
function beginInteraction(payload, mode) {
    ...
    interactionState = {
        ...
        snapTargets: buildSnapTargets(
            mode === "move" ? activeIds : [payload.widgetId],
        ),
    };
...
function buildSnapTargets(ignoreIds) {
    ...
    const xTargets = [
        0,
        props.project.meta.screenWidth / 2,
        props.project.meta.screenWidth,
    ].map((value) => ({
        value,
    }));
    const yTargets = [
        0,
        props.project.meta.screenHeight / 2,
        props.project.meta.screenHeight,
    ].map((value) => ({
        value,
    }));
...
function applyMoveSnap(nextLeft, nextTop, bounds, snapTargets) {
    ...
    return {
        left: clamp(
            snapX ? snapX.result : nextLeft,
            0,
            props.project.meta.screenWidth - bounds.w,
        ),
        top: clamp(
            snapY ? snapY.result : nextTop,
            0,
            props.project.meta.screenHeight - bounds.h,
        ),
    };
}
...
function processPointerMove(event) {
    ...
    if (interactionState.mode === "move") {
        const maxLeft =
            props.project.meta.screenWidth - interactionState.referenceBounds.w;
        const maxTop =
            props.project.meta.screenHeight -
            interactionState.referenceBounds.h;
        const nextLeft = clamp(
            interactionState.referenceBounds.x + deltaX,
            0,
            maxLeft,
        );
        const nextTop = clamp(
            interactionState.referenceBounds.y + deltaY,
            0,
            maxTop,
        );
    ...
    if (interactionState.mode === "resize") {
        ...
        widget.w = clamp(
            snapped.right - origin.x,
            120,
            props.project.meta.screenWidth - origin.x,
        );
        widget.h = clamp(
            snapped.bottom - origin.y,
            80,
            props.project.meta.screenHeight - origin.y,
        );
        updateTransformOverlay(interactionState.mode, {
            x: widget.x,
            y: widget.y,
...
function toggleRulers() {
    runMetaMutation(
        props.project.meta.showRulers === false ? "打开标尺" : "关闭标尺",
        () => {
            props.project.meta.showRulers = !(
                props.project.meta.showRulers !== false
            );
        },
    );
...
function toggleGuides() {
    runMetaMutation(
        props.project.meta.showGuides === false ? "启用参考线" : "关闭参考线",
        () => {
            props.project.meta.showGuides = !(
                props.project.meta.showGuides !== false
            );
        },
    );
    ...
    >
        <div v-if="showMeta" class="canvas-meta">
            <div>
                ...
                <p>
                    {{ project.meta.screenWidth }} x
                    {{ project.meta.screenHeight }} 画布
                </p>
            </div>
            ...
            >
                ...
                >
                    ...
                    <button
                        ...
                        @pointerdown="
                            startGuideDrag('vertical', guide.index, $event)
                        "
                    />
                    <button
                        ...
                        @pointerdown="
                            startGuideDrag('horizontal', guide.index, $event)
                        "
                    />
                    <div
                        v-if="
                            showGuides && guideDraft?.orientation === 'vertical'
                        "
                        class="canvas-reference-guide canvas-reference-guide--vertical is-draft"
                        :style="{
                            left: `${guideDraft.value}px`,
                            color: guideColor,
                        }"
                    />
                    <div
                    ...
                    <template v-for="line in measurementLines" :key="line.key">
                        ...
                        />
                        <div
                            class="canvas-measure-line__label"
                    ...
                    <div
                        v-if="transformOverlay"
                        class="canvas-transform-hud"

...
function supportsRequestBody(source) {
  return !['GET', 'HEAD'].includes(String(source.request?.method || 'GET').toUpperCase())
}
...
function fillFieldMappings(source) {
  source.request.fieldMappingsText = getRemoteFieldMappingTemplate(source.type)
}
  ...
  >
    ...
    <div class="data-source-panel__toolbar">
      ...
      <div class="data-source-panel__toolbar-actions">
        <button class="ghost data-source-panel__action-button" type="button" @click="$emit('create-source', draftType)">
          新增数据源
        </button>
        <button class="ghost data-source-panel__action-button" type="button" @click="$emit('import-source-as-new')">
          导入配置
        </button>
      </div>
    </div>
      ...
      >
        ...
        <label>
          ...
          <select v-model="source.generator">
            <option v-for="option in getGeneratorOptions(source.type)" :key="option.value" :value="option.value">
              {{ option.label }}
            </option>
          </select>
        </label>
        ...
        <div v-if="isRemoteSource(source)" class="data-source-panel__request">
          ...
          <div class="inspector-grid">
            ...
            <label>
              ...
              <select v-model="source.request.method">
                <option v-for="method in requestMethodOptions" :key="method" :value="method">
                  {{ method }}
                </option>
              </select>
            </label>
          ...
          <div class="inspector-grid">
            ...
            <label>
              <span>超时（ms）</span>
              <input v-model.number="source.request.timeout" type="number" min="500" step="500" />
            </label>
          </div>

          <label class="data-source-panel__checkbox">
            <input v-model="source.request.withCredentials" type="checkbox" />
          ...
          <div v-else-if="needsBasicAuth(source)" class="inspector-grid">
            <label>
              <span>用户名</span>
              <input
          ...
          <div v-else-if="needsCustomHeaderAuth(source)" class="inspector-grid">
            <label>
              <span>请求头名称</span>
              <input
          ...
          <div class="data-source-panel__request-actions">
            <button class="ghost" type="button" @click="fillFieldMappings(source)">填充字段模板</button>
          </div>
        ...
        <p v-if="isRemoteSource(source)" class="inspector-tip">
          字段映射左边写组件 payload 字段，右边写接口返回路径；请求预览会展示占位变量替换后的最终请求。
        </p>

        <div v-if="getRuntimeError(source.id)" class="data-source-panel__status data-source-panel__status--error">
        ...
        <div v-if="shouldShowRuntimePreview(source)" class="data-source-panel__preview">
          ...
          <div v-if="hasRequestPreview(source.id)" class="data-source-panel__preview-block">
            <div class="data-source-panel__preview-head">
              <span>最终请求预览</span>
              <button class="ghost data-source-panel__mini-button" type="button" @click="emitCopyDebug(source.id, 'request')">
                复制
              </button>
            </div>
            <textarea
          ...
          <div v-if="hasExtractedPreview(source.id)" class="data-source-panel__preview-block">
            <div class="data-source-panel__preview-head">
              <span>提取结果预览</span>
              <button class="ghost data-source-panel__mini-button" type="button" @click="emitCopyDebug(source.id, 'extracted')">
                复制
              </button>
            </div>
            <textarea
          ...
          <div v-if="hasResponsePreview(source.id)" class="data-source-panel__preview-block">
            <div class="data-source-panel__preview-head">
              <span>原始响应预览</span>
              <button class="ghost data-source-panel__mini-button" type="button" @click="emitCopyDebug(source.id, 'response')">
                复制
              </button>
            </div>
            <textarea
        ...
        <div class="data-source-panel__actions">
          ...
          <button class="ghost data-source-panel__action-button" type="button" @click="$emit('import-source', source.id)">
            导入
          </button>
          <button
          ...
          <button class="ghost data-source-panel__action-button" type="button" @click="$emit('refresh-source', source.id)">

...
const orderedLayers = computed(() =>
    [...props.widgets].sort((a, b) => b.zIndex - a.zIndex),
);
...
function isDropBefore(widgetId) {
    return (
        dragState.value.overId === widgetId &&
        dragState.value.placement === "before"
    );
}
...
function isDropAfter(widgetId) {
    return (
        dragState.value.overId === widgetId &&
        dragState.value.placement === "after"
    );
}
</script>
...
<template>
    ...
    >
        <div class="layer-list">
            ...
            >
                <button
                    class="layer-item__select"
                    type="button"
                    @click="handleSelect(widget, $event)"
                >
                    <div class="layer-item__meta">
                        ...
                        <span class="layer-item__subline"
                            >{{ widget.type }} / z{{ widget.zIndex }}</span
                        >
                    </div>
                </button>

                <div class="layer-item__actions">
                    <span v-if="widget.locked" class="layer-item__state"
                        >锁定</span
                    >
                    <button

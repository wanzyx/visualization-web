...
const provinceCoordinateMap = new Map(
    (Array.isArray(chinaGeoJson.features) ? chinaGeoJson.features : [])
        .map((feature) => {
            const normalizedName = normalizeProvinceName(
                feature?.properties?.name,
            );
            const cp = feature?.properties?.cp;
            const center = feature?.properties?.center;
            const centroid = feature?.properties?.centroid;
            const coordinate = Array.isArray(cp)
                ? cp
...
const maxValue = computed(() =>
    Math.max(1, ...items.value.map((item) => Number(item.value ?? 0))),
);
...
const statusText = computed(() => {
    ...
    return props.widget.props.enableDrilldown === false
        ? "地图概览"
        : "点击省份聚焦";
});
...
const option = computed(() => {
    const title =
        String(props.widget.props.title || "中国地图").trim() || "中国地图";
    const unit = String(props.widget.props.unit || "").trim();
    const accent =
        String(props.widget.props.accent || "#7bfecb").trim() || "#7bfecb";
    const lowColor = String(
        props.widget.props.lowColor || "rgba(70, 238, 255, 0.08)",
    ).trim();
    const highColor =
        String(props.widget.props.highColor || "#46eeff").trim() || "#46eeff";
    ...
    if (showScatter && pointItems.value.length) {
        series.push({
            ...
            symbolSize: (value, params) =>
                params?.data?.size ??
                Math.max(10, Math.sqrt(Number(value?.[2] ?? 0)) * 1.6),
...
function handleChartClick(params) {
    ...
    const nextActiveProvince =
        activeProvinceName.value === nextProvince ? "" : nextProvince;
        ...
        <div class="widget-china-map__chart">
            <div v-if="showHeader" class="widget-china-map__head">
                ...
                <div class="widget-china-map__meta">
                    <div
                        v-if="widget.props.showLegend !== false"
                        class="widget-china-map__legend"
                    >
                        <span>热度分布</span>
                        <i />
                    </div>
                    <span
                        class="widget-china-map__badge"
                        :class="{ 'is-active': Boolean(activeProvinceName) }"
                    >
                        {{ statusText }}
                    </span>
                </div>

...
const items = computed(() =>
  (Array.isArray(props.widget.props.items) ? props.widget.props.items : [])
    .map((item, index) => ({
      label: String(item?.label ?? `标签 ${index + 1}`).trim() || `标签 ${index + 1}`,
      value: String(item?.value ?? '').trim(),
      unit: String(item?.unit ?? '').trim(),
      description: String(item?.description ?? '').trim(),
      meta: String(item?.meta ?? '').trim()
    }))
    .filter((item) => item.label || item.value || item.description || item.meta)
)
...
const panelStyle = computed(() => ({
  '--tab-panel-accent': String(props.widget.props.accent || '#46eeff').trim() || '#46eeff',
  '--tab-panel-secondary':
    String(props.widget.props.secondaryColor || 'rgba(235, 247, 255, 0.16)').trim() ||
    'rgba(235, 247, 255, 0.16)'
}))
...
<template>
  <div class="widget-tab-panel" :style="panelStyle">
    <div v-if="showTitle || items.length" class="widget-tab-panel__head">
      <strong v-if="showTitle">{{ widget.props.title || widget.name }}</strong>
    ...
    <div v-if="activeItem" class="widget-tab-panel__content">
      <p v-if="activeItem.meta" class="widget-tab-panel__meta">{{ activeItem.meta }}</p>
      ...
      <p v-if="activeItem.description" class="widget-tab-panel__description">{{ activeItem.description }}</p>
    </div>

...
const frameTitle = computed(() =>
    String(props.widget.props.title || props.widget.name || "").trim(),
);
const showToolbar = computed(() => props.widget.props.showToolbar !== false);
const allowFullscreen = computed(
    () => props.widget.props.allowFullscreen !== false,
);
    ...
    <div class="widget-embed" :class="{ 'is-editor': !previewMode }">
        <div v-if="frameSrc" class="widget-embed__shell">
            ...
            <div class="widget-embed__viewport">
                ...
                <div v-if="!isLoaded" class="widget-embed__loading">
                    页面加载中...
                </div>
            ...
            <p class="widget-embed__hint">
                若页面空白，请确认目标站点允许 iframe 嵌入。
            </p>
        </div>

        <div v-else class="widget-media__empty">
            请先配置网页地址，或绑定 iframe 数据源。
        </div>
    </div>
</template>

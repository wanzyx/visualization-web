...
const categories = computed(() => {
  const source = Array.isArray(props.widget.props.categories) ? props.widget.props.categories : []
...
const colors = computed(() => {
  const source = Array.isArray(props.widget.props.colors) ? props.widget.props.colors : []
  return source.length ? source : ['#46eeff', '#7bfecb', '#ffd66b', '#6d8bff']
})

const seriesData = computed(() => {
  const items = Array.isArray(props.widget.props.items) ? props.widget.props.items : []

  if (items.length) {
    return items
      .map((item, index) => ({
        name: String(item?.name ?? item?.label ?? `项目 ${index + 1}`).trim(),
...
const hasData = computed(() => seriesData.value.some((item) => Number.isFinite(item.value) && item.value > 0))

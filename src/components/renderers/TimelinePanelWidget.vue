...
const items = computed(() =>
  (Array.isArray(props.widget.props.items) ? props.widget.props.items : [])
    .map((item, index) => ({
      time: String(item?.time ?? '').trim() || `${index + 1}`.padStart(2, '0'),
...
const themeStyle = computed(() => ({
  '--timeline-accent': String(props.widget.props.accent || '#46eeff').trim() || '#46eeff',
  '--timeline-secondary':
  ...
  <div class="widget-timeline" :style="themeStyle">
    ...
    <div v-if="items.length" class="widget-timeline__body">
        ...
        <button
          ...
          :class="[
            `is-${item.status}`,
            {
              ...
              'hide-connector': widget.props.showConnector === false
            }
          ]"
          @pointerdown.stop="stopPointer"
          @click.stop="activateItem(index, $event)"
        >
          <span class="widget-timeline__item-time">{{ item.time }}</span>
          ...
          <div class="widget-timeline__item-body">
            <div class="widget-timeline__item-top">
              ...
              <em v-if="item.tag" class="widget-timeline__item-tag">{{ item.tag }}</em>
            </div>
      ...
      <div class="widget-timeline__detail">
        ...
        <p>{{ activeItem?.description || '请在属性面板中配置时间轴节点。' }}</p>

        <div class="widget-timeline__detail-meta">
          <span class="widget-timeline__detail-chip">高亮序号 {{ activeIndex + 1 }}</span>
          <span v-if="activeItem?.tag" class="widget-timeline__detail-chip">{{ activeItem.tag }}</span>
          <span class="widget-timeline__detail-chip">状态 {{ activeItem?.status || 'pending' }}</span>
        </div>
      </div>
    </div>

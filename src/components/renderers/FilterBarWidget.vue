...
const options = computed(() =>
    (Array.isArray(props.widget.props.options)
        ? props.widget.props.options
        : []
    )
        .map((item, index) => ({
            label:
                String(item?.label ?? `选项 ${index + 1}`).trim() ||
                `选项 ${index + 1}`,
...
const activeValue = computed(() =>
    String(props.widget.props.activeValue ?? "").trim(),
);
...
<template>
    <div class="widget-filter-bar" :style="themeStyle">
        <div
            v-if="
                widget.props.showTitle !== false ||
                (options.length && widget.props.allowClear !== false)
            "
            class="widget-filter-bar__head"
        >
            <strong v-if="widget.props.showTitle !== false">{{

...
const columns = computed(() => {
  const source = Array.isArray(props.widget.props.columns) ? props.widget.props.columns : []

  return source
    .map((column, index) => ({
      key: String(column?.key ?? `column${index + 1}`).trim(),
      label: String(column?.label ?? column?.key ?? `列${index + 1}`).trim()
    }))
    .filter((column) => column.key)
})

const rows = computed(() => {
  const source = Array.isArray(props.widget.props.rows) ? props.widget.props.rows : []
  return source.map((row) => (row && typeof row === 'object' && !Array.isArray(row) ? row : {}))
})
      ...
      <div class="widget-table__body">
        ...
        >
          <span
            ...
            :class="
              column.key === 'level'
                ? ['widget-table__level', getLevelClass(row[column.key])]
                : ''
            "
          >

...
function formatNumericText(value, decimals, useGrouping) {
    ...
    const groupedIntegerPart = rawIntegerPart.replace(
        /\B(?=(\d{3})+(?!\d))/g,
        ",",
    );
...
const themeStyle = computed(() => ({
    ...
    "--digit-stat-unit-color":
        props.widget.props.unitColor || "rgba(235, 247, 255, 0.72)",
}));
</script>
...
<template>
    <div class="widget-digit-stat" :style="themeStyle">
        ...
        <div class="widget-digit-stat__body">
            <span v-if="widget.props.prefix" class="widget-digit-stat__affix">{{
                widget.props.prefix
            }}</span>
            ...
            <span v-if="widget.props.suffix" class="widget-digit-stat__affix">{{
                widget.props.suffix
            }}</span>
            <span v-if="widget.props.unit" class="widget-digit-stat__unit">{{
                widget.props.unit
            }}</span>
        </div>

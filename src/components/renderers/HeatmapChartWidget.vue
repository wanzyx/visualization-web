...
const xLabels = computed(() => {
  const source = Array.isArray(props.widget.props.xLabels) ? props.widget.props.xLabels : []
  return source.filter(Boolean)
})

const yLabels = computed(() => {
  const source = Array.isArray(props.widget.props.yLabels) ? props.widget.props.yLabels : []
  return source.filter(Boolean)
})
...
const option = computed(() => ({
  ...
  tooltip: {
    ...
    formatter: (params) => {
      ...
      const columnLabel = xLabels.value[columnIndex] ?? `列 ${columnIndex + 1}`

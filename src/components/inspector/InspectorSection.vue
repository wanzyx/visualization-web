...
onMounted(() => {
    ...
    const rawValue = localStorage.getItem(
        `inspector-section:${props.storageKey}`,
    );
...
<template>
    <section
        class="inspector-group inspector-section"
        :class="{ 'is-collapsed': !open }"
    >
        <div
            class="inspector-section__header"
            :class="{ 'has-actions': Boolean($slots.actions) }"
        >
            <button
            ...
            >
                <div class="inspector-section__copy">
                    <h3>{{ title }}</h3>
                    <p v-if="caption" class="inspector-section__caption">
                        {{ caption }}
                    </p>
                </div>
